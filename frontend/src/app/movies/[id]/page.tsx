...
export default async function MovieDetailPage({ params }: MovieDetailPageProps) {
  ...
  try {
    const [movie, rec] = await Promise.all([
      getMovie(movieId),
            ...
            <div className="grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-5">
              {rec.recommendations.map((item, index) => (
                ...
                >
                  ...
                  <p className="line-clamp-2 text-sm font-semibold md:text-base">{item.title}</p>
                  <p className="mt-2 text-xs uppercase tracking-[0.12em] text-slate-600">Mức tương đồng: {item.similarity_score}</p>
                </Link>
              ))}
            </div>
          </section>
